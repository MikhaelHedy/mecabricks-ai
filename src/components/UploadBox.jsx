...
export default function UploadBox() {
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      // Panggil fungsi global atau simpan ke store jika ini digunakan
      // di dalam App.jsx nanti akan memanggil useLegoStore.setDaeText(text)
      window.dispatchEvent(new CustomEvent('daeFileUploaded', { detail: text }));
    } catch (error) {
      console.error("Error reading file:", error);
      alert("Gagal membaca file. Pastikan itu adalah file DAE/XML yang valid.");
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-blue-400 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors duration-200">
      ...
      <p className="text-sm text-gray-500 mt-2">Ukuran file maksimal: 50MB (disarankan)</p>
    </div>
  );
}